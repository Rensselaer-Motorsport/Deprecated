/*******************************************************************************
* File Name: I2C_MPU6050_I2C_MASTER.c
* Version 2.0
*
* Description:
...
*  I2C_MPU6050_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  I2C_MPU6050_mstrRdBufPtr - used to store pointer to master write buffer.
*  I2C_MPU6050_mstrRdBufIndex - used to current index within master write
*  buffer.
*  I2C_MPU6050_mstrRdBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 I2C_MPU6050_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(I2C_MPU6050_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        /* Prepare reading */
        if(I2C_MPU6050_mstrRdBufSize < I2C_MPU6050_I2C_FIFO_SIZE) /* Reading byte-by-byte */
        {
...
uint32 I2C_MPU6050_I2CMasterSendStart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(I2C_MPU6050_CHECK_I2C_FSM_IDLE)
    {
        ...
        else
        {
            ...
            I2C_MPU6050_TX_FIFO_WR_REG = slaveAddress; /* Put address in TX FIFO */
            I2C_MPU6050_ClearMasterInterruptSource(I2C_MPU6050_INTR_MASTER_ALL);

            I2C_MPU6050_I2C_MASTER_GENERATE_START;


            while(!I2C_MPU6050_CHECK_INTR_MASTER(I2C_MPU6050_INTR_MASTER_I2C_ACK      |
            ...
            if(I2C_MPU6050_CHECK_INTR_MASTER(I2C_MPU6050_INTR_MASTER_I2C_ACK))
            {
                errStatus = I2C_MPU6050_I2C_MSTR_NO_ERROR;
            }
            else if(I2C_MPU6050_CHECK_INTR_MASTER(I2C_MPU6050_INTR_MASTER_I2C_NACK))
            {
                errStatus = I2C_MPU6050_I2C_MSTR_ERR_LB_NAK;
            }
            else if(I2C_MPU6050_CHECK_INTR_MASTER(I2C_MPU6050_INTR_MASTER_I2C_ARB_LOST))
            {
                I2C_MPU6050_state = I2C_MPU6050_I2C_FSM_IDLE;
                             errStatus = I2C_MPU6050_I2C_MSTR_ERR_ARB_LOST;
                             resetIp   = I2C_MPU6050_I2C_RESET_ERROR;
            }
            else /* I2C_MPU6050_INTR_MASTER_I2C_BUS_ERROR set is else condition */
            {
                I2C_MPU6050_state = I2C_MPU6050_I2C_FSM_IDLE;
                             errStatus = I2C_MPU6050_I2C_MSTR_ERR_BUS_ERR;
                             resetIp   = I2C_MPU6050_I2C_RESET_ERROR;
            }

            I2C_MPU6050_ClearMasterInterruptSource(I2C_MPU6050_INTR_MASTER_I2C_ACK      |
                                                        I2C_MPU6050_INTR_MASTER_I2C_NACK     |
                                                        I2C_MPU6050_INTR_MASTER_I2C_ARB_LOST |
                                                        I2C_MPU6050_INTR_MASTER_I2C_BUS_ERROR);
...
uint32 I2C_MPU6050_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(I2C_MPU6050_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        */
        ...
        while(I2C_MPU6050_CHECK_I2C_MASTER_CMD(I2C_MPU6050_I2C_MASTER_CMD_M_START))
        {
...
uint32 I2C_MPU6050_I2CMasterSendStop(void)
{
    ...
    if(I2C_MPU6050_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        */
        ...
        while(!I2C_MPU6050_CHECK_INTR_MASTER(I2C_MPU6050_INTR_MASTER_I2C_STOP     |
                                                  I2C_MPU6050_INTR_MASTER_I2C_ARB_LOST |
                                                  I2C_MPU6050_INTR_MASTER_I2C_BUS_ERROR))
        {
        ...
        if(I2C_MPU6050_CHECK_INTR_MASTER(I2C_MPU6050_INTR_MASTER_I2C_STOP))
        {
            errStatus = I2C_MPU6050_I2C_MSTR_NO_ERROR;
        }
        else if(I2C_MPU6050_CHECK_INTR_MASTER(I2C_MPU6050_INTR_MASTER_I2C_ARB_LOST))
        ...
        else /* I2C_MPU6050_INTR_MASTER_I2C_BUS_ERROR is set */
        {
            errStatus = I2C_MPU6050_I2C_MSTR_ERR_BUS_ERR;
            resetIp   = I2C_MPU6050_I2C_RESET_ERROR;
        }
...
uint32 I2C_MPU6050_I2CMasterWriteByte(uint32 theByte)
{
    ...
    if(I2C_MPU6050_CHECK_I2C_MASTER_ACTIVE)
    {
        I2C_MPU6050_TX_FIFO_WR_REG = theByte;
        ...
        else /* I2C_MPU6050_INTR_MASTER_I2C_BUS_ERROR set is */
        {
            I2C_MPU6050_state = I2C_MPU6050_I2C_FSM_IDLE;
                         errStatus = I2C_MPU6050_I2C_MSTR_ERR_BUS_ERR;
                         resetIp   = I2C_MPU6050_I2C_RESET_ERROR;
...
uint32 I2C_MPU6050_I2CMasterReadByte(uint32 ackNack)
{
    ...
    theByte = I2C_MPU6050_I2C_INVALID_BYTE;

    /* Check FSM state before read byte */
    if(I2C_MPU6050_CHECK_I2C_MASTER_ACTIVE)
    {
        while((!I2C_MPU6050_CHECK_INTR_RX(I2C_MPU6050_INTR_RX_NOT_EMPTY)) &&
              (!I2C_MPU6050_CHECK_INTR_MASTER(I2C_MPU6050_INTR_MASTER_I2C_ARB_LOST |
                                                  I2C_MPU6050_INTR_MASTER_I2C_BUS_ERROR)))
        {
        ...
        if(I2C_MPU6050_CHECK_INTR_RX(I2C_MPU6050_INTR_RX_NOT_EMPTY))
        {
            theByte = I2C_MPU6050_RX_FIFO_RD_REG;
...
void I2C_MPU6050_I2CReStartGeneration(void)
{
    ...
    uint32 status = I2C_MPU6050_I2C_STATUS_REG;

    while(I2C_MPU6050_WAIT_SDA_SET_HIGH)
    {
    ...
    if(0u != (status & I2C_MPU6050_I2C_STATUS_M_READ))
    {
        /* Generate NACK use GPIO functionality */
        I2C_MPU6050_SET_I2C_SCL_DR(I2C_MPU6050_I2C_SCL_LOW);
        CyDelayUs(I2C_MPU6050_I2C_TLOW_TIME); /* Count tLOW */

        I2C_MPU6050_SET_I2C_SCL_DR(I2C_MPU6050_I2C_SCL_HIGH);
        while(I2C_MPU6050_WAIT_SCL_SET_HIGH)
        {
            /* Wait until slave releases SCL in case if it stretches */
        }
        CyDelayUs(I2C_MPU6050_I2C_THIGH_TIME); /* Count tHIGH */
    }
